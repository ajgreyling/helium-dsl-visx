...
describe("Platform implicit fields (persistent objects)", () => {
  it("allows _id and _tstamp on persistent objects (from language metadata platformImplicitFields)", async function () {
    ...
    const testMetadata = {
      ...
      reservedIdentifiers: [],
      roleImplicitFields: [],
      platformImplicitFields: ["_id", "_tstamp"],
    };
    ...
    expect((metadata as any).platformImplicitFields.includes("_tstamp")).to.equal(true);
...
void f() {
  ...
  datetime ts = c._tstamp;
    ...
    const hasTstampError = diags.some((d: any) =>
      String(d?.message ?? "").includes("Invalid attribute/relationship `_tstamp`")
    );

    expect(hasIdError).to.equal(false, "Expected _id to be accepted on persistent objects");
    expect(hasTstampError).to.equal(false, "Expected _tstamp to be accepted on persistent objects");

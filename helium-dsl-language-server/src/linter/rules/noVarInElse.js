...
export function applyNoVarInElse(ctx) {
    ...
    for (const decl of declarations) {
        const inElse = ast.elseBlocks.some((elseRange) => rangeContains(elseRange, decl.declRange.start.line, decl.declRange.start.character));
        if (inElse) {
            pushDiagnostic(ctx, "no-var-in-else", decl.nameRange.start.line, decl.nameRange.start.character, decl.name.length, ctx.rules["no-var-in-else"].message);
        }
    }
}
function applyLegacyNoVarInElse(ctx) {
        ...
        if (inElse) {
            ...
            if (!/\breturn\b/.test(trimmed)) {
                ...
                while ((varMatch = varDeclPattern.exec(line)) !== null) {
                    pushDiagnostic(ctx, "no-var-in-else", idx, varMatch.index ?? 0, varMatch[0].length, ctx.rules["no-var-in-else"].message);
                }
            }
            if (braceDepth <= 0)
                inElse = false;

...
function summarizeDiagnostics(diagnostics: { severity?: number }[]) {
  const summary = {
  ...
  for (const diagnostic of diagnostics) {
    if (diagnostic.severity === 1) summary.errors += 1;
    else if (diagnostic.severity === 2) summary.warnings += 1;
    else if (diagnostic.severity === 3) summary.information += 1;
    else if (diagnostic.severity === 4) summary.hints += 1;
...
export function createVxmlToolHandlers(): Record<string, ToolHandler> {
  const cache = new Map<string, VxmlWorkspaceService>();
  ...
  return {
    ...
    helium_vxml_extract_unit_stubs: async (args) => {
      ...
      const existingFns = index.getUnitFunctions(unitName).map((f: FunctionDecl) => f.name);
      const existingVars = index.getUnitVariables(unitName).map((v: VariableDecl) => v.name);
...
function compactAst(ast: VxmlAst) {
  return {
    uri: ast.uri,
    view: ast.view,
...
function splitQualified(raw: string, defaultUnit: string): { unitName: string; name: string } {
  if (raw.includes(":")) {
...
function collectMissingFunctions(ast: VxmlAst, viewUnit: string, existing: string[]): string[] {
  const missing = new Set<string>();
  for (const ref of ast.references) {
    if (ref.kind !== "function") continue;
    const { unitName, name } = splitQualified(ref.name, viewUnit);
    if (unitName !== viewUnit) continue;
    if (!existing.includes(name)) missing.add(name);
  }
  if (ast.view?.initFunction) {
    const initName = splitQualified(ast.view.initFunction, viewUnit).name;
    if (!existing.includes(initName)) missing.add(initName);
  }
  return Array.from(missing.values()).sort();
}

function collectMissingVariables(ast: VxmlAst, viewUnit: string, existing: string[]): string[] {
  const missing = new Set<string>();
  for (const ref of ast.references) {

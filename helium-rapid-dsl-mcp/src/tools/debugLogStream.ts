...
import {
  ...
  type RapidProjectConfigV1,
} from "helium-dsl-language-server/api";

export type DebugStreamHandle = string;

type StreamState = {
  handle: DebugStreamHandle;
  projectRoot: string;
  configPath: string;
  wsUrl: string;
...
const streamsByRoot = new Map<string, StreamState>();
const streamsByHandle = new Map<string, StreamState>();
...
function readProjectConfig(projectRoot: string): { path: string; config: RapidProjectConfigV1 } {
  const configPath = path.join(projectRoot, RAPID_PROJECT_FILE_NAME);
  if (!fs.existsSync(configPath)) {
    throw new Error(
      `Missing ${RAPID_PROJECT_FILE_NAME} at project root: ${projectRoot}`
    );
  }
  ...
  return { path: configPath, config: parsed as RapidProjectConfigV1 };
}

function computeWsUrl(cfg: RapidProjectConfigV1): string {
  ...
  if (!appId) throw new Error("debug.appId is required in helium-rapid-dsl-project.json");
  return `${wsProtocol}//${u.host}${wsPath}?appId=${encodeURIComponent(appId)}`;
}

function computeAuthHeader(cfg: RapidProjectConfigV1): string {
  const user = (cfg.debug.heliumUser || "").trim();
  const pass = cfg.debug.heliumPassword ?? "";
  if (!user || !pass) throw new Error("debug.heliumUser and debug.heliumPassword are required.");
...
function handleSseRequest(req: IncomingMessage, res: ServerResponse, state: StreamState) {
  res.statusCode = 200;
...
function broadcast(state: StreamState, payload: unknown) {
...
export async function startDebugLogStream(opts: {
  ...
  state = {
    ...
    ws: null as any, // filled after ws constructed
    clients,
    startedAt,
  };
  ...
  ws.on("message", (data: WebSocket.RawData, isBinary: boolean) => {
    const msg = isBinary ? Buffer.from(data as any).toString("utf8") : String(data);
...
export async function stopDebugLogStream(opts: { handleOrProjectRoot: string }): Promise<{
  stopped: boolean;
  handle: string | null;
  ...
  if (!state) return { stopped: false, handle: null, projectRoot: null };

...
function parseAttributes(rawTag, tagStartOffset, toRange) {
    const attributes = [];
    ...
    while ((match = attrRegex.exec(afterName)) !== null) {
        ...
        const nameRange = toRange({ start: absoluteStart, end: absoluteStart + attrName.length });
        const value = match[4] ?? match[5];
        let valueRange;
        if (value !== undefined) {
            const quoteIndex = matchText.indexOf('"') !== -1 ? matchText.indexOf('"') : matchText.indexOf("'");
...
function appendTextSegment(stack, toRange, text, start, end) {
    if (stack.length === 0)
        return;
...
function extractViewInfo(nodes) {
...
function collectReferences(nodes) {
    const refs = [];
    ...
    for (const node of allNodes) {
        for (const attr of node.attributes) {
            ...
            if (attr.name === "init" && node.name === "view") {
                refs.push({ kind: "function", name: attr.value, range: attr.valueRange ?? attr.nameRange });
            }
            ...
            if (attr.name === "variable" && isVariableBindingNode(node.name)) {
                refs.push({ kind: "variable", name: attr.value, range: attr.valueRange ?? attr.nameRange });
            }
            if (attr.name === "name" && node.name === "attribute") {
                refs.push({ kind: "attribute", name: attr.value, range: attr.valueRange ?? attr.nameRange });
            }
...
function isVariableBindingNode(name) {
    return ["binding", "visible", "collectionSource", "content"].includes(name);
}

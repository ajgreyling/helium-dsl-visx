...
const lsRoot = path.resolve(pkgRoot, "../helium-dsl-language-server");
const bundleDir = path.join(pkgRoot, ".bundle-tmp");
...
const bundledModulePath = path.join(pkgRoot, "node_modules", "helium-dsl-language-server");
...
run(
  "npm",
  ["install", "--no-save", "--install-links=false", "--workspaces=false", "--dry-run=false", tarballPath],
  pkgRoot
);

const expectedApiPath = path.join(pkgRoot, "node_modules", "helium-dsl-language-server", "out", "src", "api.js");
